...
type IRunnerRepository interface {
	...
	// FindById 根据 ID 从底层层拉取持久化数据并转换为领域模型
...
func (repo *runnerRepository) FindById(ctx context.Context, id int64) (domain.Runner, error) {
	...
	return repo.toDomain(runner), nil
}
...
// toTagMapping 将同一脚本下所有执行器的 Tags 标签扁平合并为 TagDetail 映射映射表
func (repo *runnerRepository) toTagMapping(rs []dao.Runner) map[string]domain.TagDetail {
