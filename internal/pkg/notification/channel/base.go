...
func (s *baseChannel) Send(ctx context.Context, n notification.Notification) (notification.Response, error) {
	selector, err := s.builder.Build()
	if err != nil {
		return notification.Response{}, fmt.Errorf("%s: %w", "发送通知失败", err)
	}

	for {
		p, nextErr := selector.Next(ctx, n)
		if nextErr != nil {
			return notification.Response{}, fmt.Errorf("发送通知失败: %w", nextErr)
		}
