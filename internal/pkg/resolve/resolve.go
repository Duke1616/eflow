...
type Resolver interface {
	...
	Resolve(ctx context.Context, target Target) ([]domain.User, error)
}
...
type Engine interface {
	...
	Resolve(ctx context.Context, targets []Target) ([]domain.User, error)
	// ResolveWithErrorHandling 提供更细粒度的解析控制，支持配置 failFast（快速失败）模式
	// failFast 为 true 时，任一解析任务失败将立即取消其他进行中的任务并返回
	ResolveWithErrorHandling(ctx context.Context, targets []Target, failFast bool) ([]domain.User, error)
}
...
func (e *engine) Resolve(ctx context.Context, targets []Target) ([]domain.User, error) {
...
func (e *engine) ResolveWithErrorHandling(ctx context.Context, targets []Target, failFast bool) ([]domain.User, error) {
	if len(targets) == 0 {
		return nil, nil
	}
...
func (e *engine) mergeResults(results []resolveResult) []domain.User {
	...
	for _, result := range results {
		...
		// 比较结构体属性并合并去重
		allUsers = slice.UnionSetFunc(allUsers, result.users, func(src, dst domain.User) bool {
			return src.Id == dst.Id
		})
	}

	// 过滤并剔除空的无效数据对象
	return slice.FilterMap(allUsers, func(idx int, src domain.User) (domain.User, bool) {
		return src, src.Id != 0
	})
}
