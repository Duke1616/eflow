...
管理后台可调用 `POST /api/task/terminate` 强制终止自动化任务，请求体包含任务 `id` 和不超过
500 字的 `reason`。eflow 会先将任务置为 `CANCELLED`，再通过 etask Scheduler RPC 持久化取消
意图；提交阶段尚未取得 execution ID 时直接使用原 request ID，不会为了终止而重新创建 execution。

`CANCELLED` 是当前 Attempt 的终态，后台恢复任务不会自动重试；管理员可人工重试，eflow 会先按
原 execution ID 或 request ID 幂等确认取消意图已持久化，再创建新的 Attempt。任务与当前 Attempt
在同一事务中终止，晚到的 etask 成功或失败事件不能覆盖终止状态。撤回补偿任务不允许强制终止，
否则工单会永久停留在 `WITHDRAWING`。
...
已经进入提交或运行状态时，撤回入口仍会拒绝操作；人工强制终止与流程撤回补偿是两套独立语义，
不会由撤回请求隐式跳过正在执行的外部动作。
